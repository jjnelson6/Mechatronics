...
int8_t Accl_to_Mouse(short accl)
{
    if ( accl > 5000){
        return 6;
    }
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            if (inc < 9) {
                ...
                appData.xCoordinate = (int8_t) 0;//(int8_t) dir_table[vector & 0x07];
                appData.yCoordinate = (int8_t) 0;//(int8_t) dir_table[(vector + 2) & 0x07];
                //vector++;
                //movement_length = 0;
            }
            else{
                appData.xCoordinate = Accl_to_Mouse(Xaccl);
                appData.yCoordinate = Accl_to_Mouse(Yaccl) ;
                inc=0;
